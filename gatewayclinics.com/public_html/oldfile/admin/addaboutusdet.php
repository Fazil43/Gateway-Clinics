...
<body>
	...
	<div id="content" class="shell">
		...
		<form id="example" name="example"  onsubmit="return Inamodel_validate(this);"method="post" action="addaboutusdet.php?action=<?php echo $do;?>">
...
$details=addslashes($details1);
 ...
 $details=$details1;
		...
		$createddate=$serverdate;
			...
			$dbase_insert = "insert into $_table63(details,status,createddate)values('$details','$status','$serverdate')";
			$dbase_inserted = mysql_query($dbase_insert,$link);
 $encoded_id=base64_encode($detailsid);
$message=base64_encode('faq_details');
header("location:addaboutusdet.php?mes=$message&nid=$encoded_id&$random");
...
$detailsid=$_POST['updated_id'];
  ...
  $update="update $_table63 set details='$details',status='$status' where  id='$detailsid' ";
