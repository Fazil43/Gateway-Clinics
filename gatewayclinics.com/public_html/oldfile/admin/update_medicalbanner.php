...
<?php echo $_SESSION['SESS_USER_NAME'];?>
		...
		<form action="update_banner.php?action=<?php echo $do;?>"method="post" enctype="multipart/form-data" name="example" id="example"  onsubmit="return Inamodel_validate(this);">
...
$createddate=date('Y-m-d');
	...
	//To Upload
	$your_image->uploadTo = 'uploads2/';
	$upload = $your_image->upload($_FILES['image']);
	echo "<div>" . $upload . "</div>";
	
	//To Resize
	$your_image->newPath = 'thumbs2/';
...
$Upinsql="update $_table59 set image='$resized' where id='$newsid'";
...
header("location:list_medicalbanner.php?srid=$encoded_id");
