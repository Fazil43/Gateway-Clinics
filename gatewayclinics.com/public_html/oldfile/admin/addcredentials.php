...
$decodednewsid=base64_decode($gettingproductnameid);
 ...
 $sel="SELECT  * FROM  $_table6 where id='$decodednewsid' ";
...
<body>
	...
	<div id="content" class="shell">
		...
		<form id="example" name="example"  onsubmit="return Inamodel_validate(this);"method="post" action="addcredentials.php?action=<?php echo $do;?>">
...
<?php echo $details;?></textarea>
  ...
  generate_wysiwyg('textarea1');
		    ...
		    <tr>
		      <td>&nbsp;</td>
		      <td><input type="submit" name="Submit" value="<?php echo $do;?>Credentials" /></td>
	        </tr>
		    <tr>
		      <td>&nbsp;</td>
		      <td>&nbsp;</td>
...
if (!get_magic_quotes_gpc()) {
    ...
    $details=$details1;
		...
		$createddate=$serverdate;
			...
			$dbase_insert = "insert into $_table6(details,status,createddate)values('$details','$status','$serverdate')";
			$dbase_inserted = mysql_query($dbase_insert,$link) or die('error');
 $encoded_id=base64_encode($detailsid);
$message=base64_encode('faq_details');
header("location:addcredentials.php?mes=$message&nid=$encoded_id&$random");
...
$detailsid=$_POST['updated_id'];
  ...
  $update="update $_table6 set details='$details',status='$status' where  id='$detailsid' ";
