...
<body>
	...
	<div id="content" class="shell">
		...
		<form id="example" name="example"  onsubmit="return Inamodel_validate(this);"method="post" action="addfaq.php?action=<?php echo $do;?>">
...
<?php echo $details;?></textarea>
  ...
  generate_wysiwyg('textarea1');
		    ...
		    <tr>
		      <td>&nbsp;</td>
		      <td><input type="submit" name="Submit" value="<?php echo $do;?>Faq" /></td>
	        </tr>
		    <tr>
		      <td>&nbsp;</td>
		      <td>&nbsp;</td>
...
if($action=='Create')
{
	...
	if(isset($_POST['Submit']))
	{
		...
		$createddate=$serverdate;
			...
			$dbase_insert = "insert into $_table6(title,details,status,createddate)values('$title','$details','$status','$serverdate')";
			$dbase_inserted = mysql_query($dbase_insert,$link);
 $encoded_id=base64_encode($detailsid);
$message=base64_encode('faq_details');
header("location:addfaq.php?mes=$message&nid=$encoded_id&$random");
...
$detailsid=$_POST['updated_id'];
  ...
  $update="update $_table6 set details='$details',title='$title',status='$status' where  id='$detailsid' ";
