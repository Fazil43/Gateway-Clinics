...
<body>
	...
	<div id="content" class="shell">
		...
		<form id="example" name="example"  onsubmit="return Inamodel_validate(this);"method="post" action="addsocial.php?action=<?php echo $do;?>">
...
<?php echo $details;?></textarea>
  ...
  generate_wysiwyg('textarea1');
		    ...
		    <tr>
		      <td>&nbsp;</td>
		      <td><input type="submit" name="Submit" value="<?php echo $do;?>Social" /></td>
	        </tr>
		    <tr>
		      <td>&nbsp;</td>
		      <td>&nbsp;</td>
...
$details=addslashes($details1);
 ...
 $details=$details1;
		...
		$createddate=$serverdate;
			...
			$dbase_insert = "insert into $_table3(category,details,status,createddate)values('$category','$details','$status','$serverdate')";
			$dbase_inserted = mysql_query($dbase_insert,$link);
 $encoded_id=base64_encode($detailsid);
$message=base64_encode('faq_details');
header("location:addsocial.php?mes=$message&nid=$encoded_id&$random");
...
$detailsid=$_POST['updated_id'];
  ...
  $update="update $_table3 set category='$category',details='$details',status='$status' where  id='$detailsid' ";
