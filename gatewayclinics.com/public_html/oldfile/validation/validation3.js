window.addEvent('domready', function() {
	...
	if (form) {
		...
		form.addEvent('submit', function (e) {
			...
			if (complaint.get('value').length === 0) 
			{
				isValid = false;
				addError(complaint, complaintError);
			} else {
				isValid = true;						
				removeError(complaint);
			}
			
			
			// Test Address length
			if (address.get('value').length === 0) {
				isValid = false;
				addError(address, addressError);
			} else {
				isValid = true;
				removeError(address);
			}
