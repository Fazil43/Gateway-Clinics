...
function SidebarToNavigation_bootstrap($page, $currentpage, $pagesSorted, $level,$i) {
    // Make sure there's both a menu and title attribute
    if ($page['menu'] == '') { $page['menu'] = $page['title']; }
    if ($page['title'] == '') { $page['title'] = $page['menu']; }
    ...
    if (count($Children) == 0) {
        // Just a regular link, no children
        $link = '<a href="' . find_url($page['url'], $page['parent']) . '" title="' . encode_quotes(cl($page['title'])) . '" class="list-group-item">' . strip_decode($page['menu']) . '</a>';
        $submenu = '';
    } else {
        // We have children, create a submenu
        $caret = ($level == 1) ? '<b class="caret"></b>' : '';
        $link = '<a href="#submenu'.$i.'" title="' . encode_quotes(cl($page['title'])) . '" data-toggle="collapse" class="list-group-item" >' . strip_decode($page['menu']) . $caret . '</a>';

        $submenu = '<div class="collapse list-group-submenu" id="submenu'.$i.'">';

        foreach ($pagesSorted as $Child) {
            if ((in_array($Child['url'], $Children)) && ($Child['menuStatus'] == 'Y') && (($Child['private'] != 'Y') || ((isset($USR) && $USR == get_cookie('GS_ADMIN_USERNAME'))))) {
              $submenu .= SidebarToNavigation_bootstrap($Child, $currentpage, $pagesSorted, $level + 1,$i);
            }
        }
...
function get_bootstrap($currentpage) {
    ...
    if (count($pagesSorted) != 0) {
        foreach ($pagesSorted as $page) {
            if ((!$page['parent']) && ($page['menuStatus'] == 'Y') && (($page['private'] != 'Y') || ((isset($USR) && $USR == get_cookie('GS_ADMIN_USERNAME'))))) {
                // Append to the menu string
                $menu .= SidebarToNavigation_bootstrap($page, $currentpage, $pagesSorted, 1,$i);
            }
            $i++;
        }
    }
...
function FooterToNavigation_bootstrap($page, $currentpage, $pagesSorted, $level,$i) {
    // Make sure there's both a menu and title attribute
    if ($page['menu'] == '') { $page['menu'] = $page['title']; }
    if ($page['title'] == '') { $page['title'] = $page['menu']; }
    ...
    if (count($Children) == 0) {
        // Just a regular link, no children
        $link = '<div class="col-md-4"><a href="' . find_url($page['url'], $page['parent']) . '" title="' . encode_quotes(cl($page['title'])) . '" class="footer-link">' . strip_decode($page['menu']) . '</a></div>';
        $submenu = '';
    } else {
        // We have children, create a submenu
        $caret = ($level == 1) ? '' : '';
        $link = '<div class="col-md-4"><a href="#submenu'.$i.'" title="' . encode_quotes(cl($page['title'])) . '" class="footer-link" >' . strip_decode($page['menu']) . $caret . '</a></div>';

        //$submenu = '<div class="list-group-submenu" id="submenu'.$i.'">';

        foreach ($pagesSorted as $Child) {
            if ((in_array($Child['url'], $Children)) && ($Child['menuStatus'] == 'Y') && (($Child['private'] != 'Y') || ((isset($USR) && $USR == get_cookie('GS_ADMIN_USERNAME'))))) {
              $submenu .= FooterToNavigation_bootstrap($Child, $currentpage, $pagesSorted, $level + 1,$i);
            }
        }
...
function get_allPages($currentpage) {
    ...
    if (count($pagesSorted) != 0) {
        foreach ($pagesSorted as $page) {
            if ((!$page['parent']) && ($page['menuStatus'] == 'Y') && (($page['private'] != 'Y') || ((isset($USR) && $USR == get_cookie('GS_ADMIN_USERNAME'))))) {
                // Append to the menu string
                $menu .= FooterToNavigation_bootstrap($page, $currentpage, $pagesSorted, 1,$i);
            }
            $i++;
        }
    }
